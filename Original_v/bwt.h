...
inline unsigned int find_occ(unsigned int line, int delta, bwt_string_type *bwt, high_occ_table_type* high_occ_table)
{
	...
	unsigned int* sp, unsigned int* ep)
{
	...
	for (; j >= 0; j--)
	{
		...
		top = find_occ(top, bitmapper_index_params.delta, bitmapper_index_params.bwt, bitmapper_index_params.high_occ_table);
		bot = find_occ(bot, bitmapper_index_params.delta, bitmapper_index_params.bwt, bitmapper_index_params.high_occ_table);
...
inline unsigned int get_sa(SA_flag_string_type* SA_flag, unsigned int line, bwt_string_type *bwt, unsigned int *sa, high_occ_table_type* high_occ_table)
{
	unsigned int l = line;
	...
	while (((SA_flag[actually_line] << (last & 63))&bitmapper_index_params.mode_SA_flag) == 0)
	{
		...
		**/
		...
		actually_line = actually_line + (unsigned int)(last >> 6);
